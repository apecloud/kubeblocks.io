...
checkExistingCLI() {
    if [ -f "$CLI_FILE" ]; then
        echo -e "\napepipe is detected: $CLI_FILE"
        echo -e "\nPlease uninstall first"
        exit 1
    elif [ -f "$CLI_BREW_FILE" ]; then
        echo -e "\napepipe is detected: $CLI_BREW_FILE"
        echo -e "\nPlease uninstall first"
        exit 1
    else
...
installFile() {
    ...
    if [ $? -eq 0 ] && [ -f "$CLI_FILE" ]; then
        ...
        apepipe version
        echo -e "Make sure your docker service is running and begin your journey with apepipe:\n"
...
fail_trap() {
    ...
    if [ "$result" != "0" ]; then
        ...
        echo "Go to https://ape.io for more support."
    fi
    cleanup
    exit $result
}
